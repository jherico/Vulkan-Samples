...
void HPPRenderPass::create_renderpass(const std::vector<vkb::rendering::HPPAttachment> &attachments, const std::vector<vkb::common::HPPLoadStoreInfo> &load_store_infos, const std::vector<HPPSubpassInfo> &subpasses)
{
	...
	for (size_t i = 0; i < subpasses.size(); ++i)
	{
		...
		{
			auto default_layout = vkb::is_depth_format(attachment_descriptions[i_attachment].format) ? vk::ImageLayout::eDepthStencilAttachmentOptimal : vk::ImageLayout::eShaderReadOnlyOptimal;
		...
		if (!subpass.disable_depth_stencil_attachment)
		{
			...
			auto it = find_if(attachments.begin(), attachments.end(), [](const vkb::rendering::HPPAttachment attachment) { return is_depth_format(attachment.format); });
...
HPPRenderPass::HPPRenderPass(Device &device, const std::vector<vkb::rendering::HPPAttachment> &attachments, const std::vector<vkb::common::HPPLoadStoreInfo> &load_store_infos, const std::vector<HPPSubpassInfo> &subpasses) :
    HPPVulkanResource{VK_NULL_HANDLE, &device},
    ...
    color_output_count{}
{
	...
	{
		create_renderpass<vk::SubpassDescription2KHR, vk::AttachmentDescription2KHR, vk::AttachmentReference2KHR, vk::SubpassDependency2KHR, vk::RenderPassCreateInfo2KHR>(attachments, load_store_infos, subpasses);
	}
	else
	{
		create_renderpass<vk::SubpassDescription, vk::AttachmentDescription, vk::AttachmentReference, vk::SubpassDependency, vk::RenderPassCreateInfo>(attachments, load_store_infos, subpasses);
	}
}
...
HPPRenderPass::~HPPRenderPass()
{
	...
	if (get_handle())
	{
		get_device().get_handle().destroy(get_handle());
	}
}
