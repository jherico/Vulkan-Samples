...
class HPPStats
{
	using StatGraphData = vkb::StatGraphData;

  public:
	/**
	 * @brief Constructs a HPPStats object
	 * @param render_context The RenderContext for this sample
	 * @param buffer_size Size of the circular buffers
	 */
	explicit HPPStats(size_t buffer_size = 16);

	/**
	 * @brief Destroys the HPPStats object
	 */
	~HPPStats();
  ...
  private:
	/// HPPStats that were requested - they may not all be available
